...
5. **담당자(`/today`) UI 원칙**: 60대 사용자 기준 — 큰 글자, 화면당 결정 1개, 어떤 기록도 탭 2회 이내 완료 (PRD §9). 관리자 화면에는 이 제약이 없다. 화면 디자인은 Figma `junction`을 따르되 **문구는 `domain.ts` 상수**를 쓴다 — 의도적으로 다른 지점 목록은 [ADR-0014](docs/adr/0014-figma-design-with-domain-terms.md)·[ADR-0020](docs/adr/0020-visit-record-flow.md)
