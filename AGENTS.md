...
- 용어 통일: 담당자(생활지원사) / 관리자 / 대상자 / 경보 단계(주의·경계·비상) / 위험 단계(심각·경계·주의) / 승격(방문 큐로) / 맥락 브리핑(FR-12 — 문서·코드·PR의 개념 이름. `AI 분석`이라 부르지 않는다. **화면 문구만 예외로 Figma 167:10322의 `AI 요약`**이며, 그 문구도 `SUBJECT_INFORMATION_LABELS.BRIEFING_TAB` 한 곳에서만 나온다)
